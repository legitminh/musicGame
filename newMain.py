...
mode_type = tuple[Callable, list[parameters]]
...
def level_select():
    ...
    for _i in range(level_amount):
        try:
            ...
            name = 0
        high_score_l.append(
            Button(screen, [screen.get_width() - 100, _i * 60 + 95 - up], _i, round(float(name), 2), 30, alignment_pos='topright')
        )
    ...
    for _i in range(level_amount):
        ...
        _i = int(_i.replace('e', ''))
        stars_l.append(
            Button(screen, [screen.get_width(), _i * 60 + 95 - up], None, star, 30, path='Assets/star.png',
    ...
    others_l = [
        Button(screen, [screen.get_width() / 2, 50], None, 'Choose a Level!', 50),
        Button(screen, [screen.get_width(), 10], None, stars, 50, alignment_pos='topright')
    ]
    ...
    for _i in range(level_amount):
        try:  # if song is written
            ...
            directory = songs[_i]
            if locked:
                lock_l.append(Button(screen, [10, _i * 60 + 95 - up], None, requirements[_i], 30, alignment_pos='topleft',
        ...
        level_select_l.append(
            Button(screen, [100, _i * 60 + 95 - up], "To be discovered" if locked else _i, name, 30, alignment_pos='topleft'))
    lock_g.add(lock_l)
                ...
                if event.button == 1:
                    ...
                    for i in range(len(level_select_g.sprites())):
                        ...
                        if s.rect.collidepoint(clicked_pos) and isinstance(level_select_l[i].mode_c, int):
                            return (mode_choice, [str(level_select_l[i].mode_c)])
...
def mode_choice(which: str):
    which = str(which)
    # modes
    modes_l = [
        Button(screen, [screen.get_width() / 2, screen.get_height() / 2 - 130], None, 'Speed', 30),
...
def outro(level_num, score, slow_down):
