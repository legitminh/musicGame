...
def main():
    ...
    while True:  # Playing >> End screen
        if mode == "intro": #if is intro state
            mode = intro()
        elif mode == 'level selection': #if is menu state
            mode = level_select()
        elif mode == 'options': #if is game settings state
            mode = options() 
        elif isinstance(mode, int): #if mode is a level ID
