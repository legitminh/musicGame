"""
TODO:
 - slowdown music choosing and loading
 - extreme mode
 - Reduce time needed to change to level selection
 - Add a program that checks the level files for errors
 - Add a program that makes changing/adding features simpler in Note Compiler
...
def intro():
    while True:
        ...
        intro_l = [
            Button(screen, [screen.get_width() // 2, screen.get_height() // 2 - 50], '', 'A Random Piano Game', 50),
...
def options():
    ...
    while True:
        screen.fill('light gray')
        volume = int(sound_slider.rect.topleft[0] - sound_slider.start_pos[0]) / (sound_slider.end_pos[0] - sound_slider.start_pos[0] - sound_slider.rect.width)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                file_writer()
                pygame.quit()
...
def mode_choice(which):
    # modes
    modes_l = [
        Button(screen, [screen.get_width() / 2, screen.get_height() / 2 - 115], '', 'Slowdown', 30),
    ...
    # practice slider
    mouse_down = False
    slider = ScrollBar(screen, [modes_l[0].rect.bottomleft[0], modes_l[0].rect.bottomleft[1] + 10],
                    [modes_l[0].rect.bottomright[0], modes_l[0].rect.bottomright[1] + 50], [75, 40],
                    'black', 'dark gray', orientation='horizontal'
                    )
    slider_g = pygame.sprite.GroupSingle(slider)
    # lock
    lock_g = pygame.sprite.GroupSingle()
    if high_scores[which] != 100:
        lock_g.add(Button(screen, modes_l[2].rect.topleft, '', 5, 30, path='Assets/lock.png',
                        dim=(50, 50), alignment_pos='topright'))
    framerate = 20
    while True:
        slow_down = .95 * ((slider.rect.topleft[0]) / (slider.end_pos[0] - slider.start_pos[0] - 75) - .05 - 3.8)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
                end()
                ...
                for i, button in enumerate(modes_l):
                    button.rect.center = screen.get_width() / 2, screen.get_height() / 2 + 50*(i-1) - (0 if i else 65)
                slider.start_pos = modes_l[0].rect.bottomleft
                slider.end_pos = [modes_l[0].rect.bottomright[0], modes_l[0].rect.bottomright[1]]
                slider.update(screen_change=True)
                slider_g.sprite.rect.topleft = \
                    slider.start_pos[0] + (slow_down * (slider.end_pos[0] - slider.start_pos[0])), slider.start_pos[1]
            elif event.type == pygame.KEYDOWN:
                if event.key == pygame.K_ESCAPE:
                ...
                if event.button == 1:
                    ...
                    for i in range(len(modes_g.sprites())):
                        if modes_g.sprites()[i].rect.collidepoint(clicked_pos):
                            ...
                            elif 'e' not in str(modes_l[i].mode_c):
                                return modes_l[i].mode_c, slow_down
            elif event.type == pygame.MOUSEBUTTONUP:
                mouse_down = False
        if mouse_down:
            slider.click_drag(pygame.mouse.get_pos())
        screen.fill('light gray')
        if high_scores[which] != 100:
            lock_g.draw(screen)
            lock_g.update()
        slider_g.update(f'{int(slow_down * 100)}%', args_c='white')
        modes_g.draw(screen)
        modes_g.update()
        pygame.display.update()
        clock.tick(framerate)
...
def extreme_level(which, slowdown) -> tuple[str, float] | str:
    ...
    for i, note in enumerate(notes):
        note[0] = note_change[note[0]][random.randint(0, 3)]
    correct_hits = 0
    total_hits = 0
    last_time = pygame.time.get_ticks()
    framerate = 50
    ...
    def all_note_cycle():  # note GFX
        ...
        for note in notes_copy:  # check list representing note
            ...
            make_text(note_numbers[note[0]] * screen.get_width() / 10 + screen.get_width() / 20,
                    screen.get_height() * .8 - note[2] - note[1] * .5,
                    note[0])
    ...
    while True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if first:
                    if (1 - slowdown) < .99:
                        pygame.mixer.music.load(songs[which].replace("Musics/","ProcessedMusics/").replace(".wav",f'{slowdown}.wav'))
                    else:
...
def level(which, slowdown) -> tuple[int, float] | str:
    ...
    while True:
        ...
        for i, _ in enumerate(notes):
            if 0 > notes[i][2] + notes[i][1]:
                # print('fail')
                total_hits += 1
                remove_from += 1
            else:
                break
        notes = notes[remove_from:]
        for a in holds.keys():
            if type(holds[a]) is float:  # is being held
                if holds[a] > 0:
                    holds[a] -= dt
    ...
    with open(f'MachineNotes/{which_level}.txt') as f:
        ...
        for _ in range(times):
            ...
            for line in machine_notes:  # a#-#
                if line[0] == '|':
                    multi = eval(line[1:line.find('-')]) / (1 - slow_down)
                elif line[0].isalpha():
                    ...
                    leng = float(line[1:line.find("-")]) * multi
                    dist_from_bottom += leng * 60
    notes.sort(key=lambda notes: notes[2])
    return notes
